"friends/ChatRoomDlg.res"
{
	...
	styles
	{
		...
		CChatActionsButton
		{
			...
			render {
				// lines around
				1="fill( x0 + 6, y0 +4, x1, y0 +5, ButtonBorder )"  // top
				2="fill( x0 + 6, y1 -5 , x1, y1 -4, ButtonBorder )"  // bottom
				3="fill( x0 +5, y0 + 5, x0 +6, y1 - 5, ButtonBorder )"  // left
				4="fill( x1 , y0 +5, x1 + 1, y1 -5, ButtonBorder )"  // right
	
				// single pixel fills in the corners
				5="fill( x0 + 6, y0 + 5 , x0 + 7, y0 + 6, ButtonBorder )" // top left
				6="fill( x1 - 1, y0 + 5, x1, y0 + 6, ButtonBorder )" // top right 
				7="fill( x0 + 6, y1 - 6, x0 + 7, y1 - 5, ButtonFace2 )" // bottom left
				8="fill( x1 - 1, y1 - 6, x1, y1 - 5, ButtonFace2 )" // bottom right

				9="fill( x0 + 6, y0 + 4 , x0 + 7, y0 + 5, ButtonFace2 )" // top left
				10="fill( x1 - 1, y0 + 4, x1, y0 + 5, ButtonFace2 )" // top right 
				11="fill( x0 + 6, y1 - 5, x0 + 7, y1 - 4, ButtonFace2 )" // bottom left
				12="fill( x1 -1, y1 - 5, x1, y1 - 4, ButtonFace2 )" // bottom right

				13="fill( x0 + 5, y0 + 5 , x0 + 6, y0 + 6, ButtonFace2 )" // top left
				14="fill( x1, y0 + 5, x1 + 1, y0 + 6, ButtonFace2 )" // top right 
				15="fill( x0 + 5, y1 - 6, x0 + 6, y1 - 5, ButtonFace2 )" // bottom left
				16="fill( x1, y1 - 6, x1 + 1, y1 - 5, ButtonFace2 )" // bottom right
				}
		}
